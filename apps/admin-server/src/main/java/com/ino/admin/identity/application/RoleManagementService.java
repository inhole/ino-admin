...
public class RoleManagementService implements RoleManagementUseCase {
    ...
    public UpdatedRole replacePermissions(String roleKey, List<String> permissions) {
        if ("SUPER_ADMIN".equals(roleKey))
            throw new BusinessException(ErrorCode.SYSTEM_ROLE_PROTECTED, "최고 관리자 권한은 변경할 수 없습니다.");
    ...
    public RoleView create(CreateRole command) {
        ...
        if (!key.matches("[A-Z][A-Z0-9_]{2,49}")) throw new BusinessException(ErrorCode.INVALID_ROLE_KEY);
        if (roleRepository.existsById(key)) throw new BusinessException(ErrorCode.ROLE_ALREADY_EXISTS);
    ...
    private Role editableCustomRoleForUpdate(String key) {
        var role = roleRepository.findByIdForUpdate(key)
                .orElseThrow(() -> new BusinessException(ErrorCode.ROLE_NOT_FOUND));
        if (role.systemRole()) throw new BusinessException(ErrorCode.SYSTEM_ROLE_PROTECTED);
        return role;
    }

    private Role editableCustomRole(String key) {
        var role = roleRepository.findById(key).orElseThrow(() -> new BusinessException(ErrorCode.ROLE_NOT_FOUND));
        if (role.systemRole()) throw new BusinessException(ErrorCode.SYSTEM_ROLE_PROTECTED);
        return role;
    }
    ...
    private Set<String> validatedPermissions(List<String> permissions) {
        var allowed = Arrays.stream(Permission.values()).map(Permission::key).collect(Collectors.toSet());
        var requested = Set.copyOf(permissions);
        if (!allowed.containsAll(requested)) throw new BusinessException(ErrorCode.INVALID_PERMISSION);
        return requested;
    }
