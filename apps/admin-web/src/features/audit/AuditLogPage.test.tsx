...
beforeEach(() => {
  ...
  auditApi.getAuditLogs.mockResolvedValue({
    content: [{ id: "audit-1", actorId: "actor-1", action: "PATCH",
...
test("shows audit records and applies filters", async () => {
  ...
  fireEvent.click(screen.getByRole("combobox", { name: "동작" }));
  const patch = await screen.findByRole("option", { name: "PATCH" });
  fireEvent.pointerDown(patch, { pointerType: "mouse" });
  fireEvent.click(patch);
  fireEvent.click(screen.getByRole("button", { name: "검색" }));

  await waitFor(() => expect(auditApi.getAuditLogs).toHaveBeenLastCalledWith(
    expect.objectContaining({ actorId: "actor-1", action: "PATCH" }),
  ));
});
