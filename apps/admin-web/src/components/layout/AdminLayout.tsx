...
export function AdminLayout() {
  ...
  return <div className="grid min-h-svh md:grid-cols-[260px_1fr]">
    <aside aria-label="주 메뉴" className="flex flex-col border-r bg-card p-5">
      ...
      <nav className="grid gap-1"><NavLink className={linkClass} end to="/"><LayoutDashboard size={18} />대시보드</NavLink>{user?.permissions.includes('user:read') && <NavLink className={linkClass} to="/users"><Users size={18} />사용자</NavLink>}{user?.permissions.includes('permission:read') && <NavLink className={linkClass} to="/permissions"><KeyRound size={18} />권한</NavLink>}</nav>
