...
import { ChevronLeft, ChevronRight, File, KeyRound, LayoutDashboard, LogOut, Menu, PanelLeft, Users } from 'lucide-react'
...
export function AdminLayout() {
  ...
  const sidebar = (mobile = false) => <div className="flex h-full flex-col">
    <div className={cn('flex h-16 items-center border-b border-sidebar-border px-4', collapsed && !mobile ? 'justify-center' : 'justify-between')}>{(!collapsed || mobile) && <span className="font-bold tracking-tight text-sidebar-foreground">{t('appName')}</span>}</div>
  ...
  return <div className={cn('min-h-svh md:grid', collapsed ? 'md:grid-cols-[76px_1fr]' : 'md:grid-cols-[260px_1fr]')}><a className="skip-link" href="#main-content">{t('skipToContent')}</a><aside className="sticky top-0 hidden h-svh bg-sidebar transition-[width] md:block">{sidebar()}<Button aria-label={collapsed ? '사이드바 펼치기' : '사이드바 접기'} className="absolute -right-3 top-20 size-7 rounded-full shadow-sm" onClick={() => setCollapsed(value => !value)} size="icon" variant="outline">{collapsed ? <ChevronRight size={15} /> : <ChevronLeft size={15} />}</Button></aside><div className="min-w-0"><header className="safe-top sticky top-0 z-30 flex min-h-16 items-center justify-between border-b bg-background/90 px-4 backdrop-blur md:hidden"><Button aria-label={t('openMenu')} onClick={() => setMobileOpen(true)} size="icon" variant="ghost"><PanelLeft /></Button><span className="font-bold">{t('appName')}</span><span aria-hidden="true" className="size-9" /></header><main className="mx-auto w-full max-w-[1440px] px-4 py-6 sm:px-6 md:px-8 md:py-10" id="main-content" tabIndex={-1}><Outlet /></main></div><Sheet onOpenChange={setMobileOpen} open={mobileOpen} showClose={false} title={t('navigation')}><aside className="h-full bg-sidebar">{sidebar(true)}</aside></Sheet></div>
}
