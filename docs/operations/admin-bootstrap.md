...
저장소 루트의 선택적 `.env` 파일을 사용할 때는 파일을 BOM 유무와 관계없이 반드시 **UTF-8**로 저장한다. 터미널의 콘솔 코드 페이지에 의존해 파일을 작성하거나 변환하지 않는다. 서버는 `.env`를 UTF-8로 읽고, OS 또는 배포 환경이 주입한 환경 변수는 `.env`보다 우선한다.
